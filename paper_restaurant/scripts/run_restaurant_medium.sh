...
OUT_ROOT="${OUT_ROOT:-runs/talukder_scale_medium}"
...
for SEED in $(seq "${SEED_FROM}" "${SEED_TO}"); do
  python -m anticipatory_rl.agents.restaurant_dqn \
    --run-label talukder_medium_myo_seed"${SEED}" \
  ...
  python -m anticipatory_rl.agents.restaurant_dqn \
    --run-label talukder_medium_ant_seed"${SEED}" \
    --config-path anticipatory_rl/configs/restaurant_symbolic.yaml \
    --layout-corpus "${CORPUS_PATH}" \
    --sample-layout-per-reset \
    --task-sequence-length 40 \
...
python paper_restaurant/scripts/restaurant_multi_seed_infer.py \
  --anticipatory-weights runs/talukder_medium_ant_seed0/restaurant_dqn.pt \
  --myopic-weights runs/talukder_medium_myo_seed0/restaurant_dqn.pt \
  --config-path anticipatory_rl/configs/restaurant_symbolic.yaml \
  --layout-corpus "${CORPUS_PATH}" \
  --sample-layout-per-reset \
  --eval-layout-count 100 \
  --task-sequence-length 40 \
  --num-tasks 4000 \
  --tasks-per-reset 40 \
  --seed-from "${SEED_FROM}" \
