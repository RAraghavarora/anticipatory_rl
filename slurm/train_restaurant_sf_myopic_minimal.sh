...
# Run training on minimal environment (2 locations, 1 object)
srun python -m anticipatory_rl.agents.restaurant.sf_dqn \
    --total-steps 500000 \
    --myopic \
    --max-steps-per-task 24 \
    --config-path anticipatory_rl/configs/restaurant/restaurant_symbolic_minimal.yaml \
    --output-name "restaurant_sf_myopic_minimal.pt" \
    --run-label "sf_myopic_minimal_$(date +%Y%m%d_%H%M%S)"
