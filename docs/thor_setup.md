...
```bash
python tests/test_thor_env.py --scene FloorPlan1 --episodes 2 --max-steps 150
```
...
```bash
python tests/test_thor_env.py --house-glob 'data/procthor/*.json' --episodes 1
```
...
Refer back to `tests/test_thor_env.py` for a complete list of switches.
