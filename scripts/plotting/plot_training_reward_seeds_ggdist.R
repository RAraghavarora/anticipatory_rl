# Learning curve (task return vs. training step) aggregated across all 4
# checkpoint seeds {0,4,8,16}, myopic vs anticipatory.
#
# Each seed's own binned mean-task-return curve is one "draw" at each x
...
csv_path <- "results/canonical_planner/figures/training_reward_seeds_binned.csv"
out_path <- "results/canonical_planner/figures/training_reward_seeds_ggdist.png"
...
p <- ggplot(df, aes(x = env_step, y = mean_value, fill = label, color = label)) +
  ...
  labs(
    x = "Training step",
    y = "Task return",
    title = "Learning curves aggregated across seeds",
