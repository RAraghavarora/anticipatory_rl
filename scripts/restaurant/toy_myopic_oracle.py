...
def run_myopic_oracle(
    *,
    ...
    for t_idx in range(num_tasks):
        task = sample_task(env, uniform_task_type_prob=0.5)

        # Resample pick_place if target object is infrastructure or contained.
        for _ in range(20):
            ...
            if obj_name is None:
                break
            obj = env.state.objects.get(obj_name)
            if obj is None:
                break
        ...
        if result.success and steps > 0:
            for i, (name, args) in enumerate(result.plan_actions):
